...
_CMD_UTIL_FLAGS = {(1, False): "--uint8", (1, True): "--int8", (2, False): "--uint16",
                   (2, True): "--int16", (4, False): "--uint32", (4, True): "--int32",
...
class _Parameter:
    ...
    def __init__(self, name: str, resolved: ResolvedType, description: str):
        ...
        if resolved.kind == "float":
            ...
            description += f" [bool: enter {BOOL_FALSE} for FALSE, {BOOL_TRUE} for TRUE]"
        elif resolved.kind == "enum":
            values = ", ".join(f"{name}={value}" for name, value in (resolved.enum_constants or {}).items())
...
def _command_parameters(dictionary: Dictionary, command: Command, descriptor: ResolvedType,
                        opcode_type: ResolvedType) -> List[_Parameter]:
    """Build the parameter list for a command: descriptor, opcode, then arguments"""
    parameters = [
        _Parameter(
            "FramingDescriptor", descriptor,
    ...
    for param in command.params:
        ...
        parameters.append(_Parameter(param["name"], resolved, param.get("annotation") or param["name"]))
    return parameters


def generate_commands(dictionary: Dictionary, cmd_gui_dir: Path,
                      command_host: str = "127.0.0.1", command_port: int = 1234) -> Tuple[int, List[str]]:
    ...
    descriptor = dictionary.resolve_definition(dictionary.type_definitions["FwPacketDescriptorType"])
    opcode_type = dictionary.resolve_definition(dictionary.type_definitions["FwOpcodeType"])
    message_id = (
        dictionary.get_apid("FW_PACKET_COMMAND") | STREAM_ID_TYPE_COMMAND | STREAM_ID_SEC_HDR
    )
    ...
    for command in dictionary.get_commands():
        try:
            parameters = _command_parameters(dictionary, command, descriptor, opcode_type)
