...
Os::Task::Status CfsTask::start(const Arguments &arguments)
{
    ...
    // OSAL limits task names to OS_MAX_API_NAME including the terminator; keep
    // the trailing characters when truncating, as fully-qualified F Prime names
    // share a topology prefix and the tail carries the distinguishing name
    const char      *name       = arguments.m_name.toChar();
    const FwSizeType nameLength = arguments.m_name.length();
    if (nameLength >= static_cast<FwSizeType>(OS_MAX_API_NAME))
    {
        name = &name[nameLength - (OS_MAX_API_NAME - 1)];
        Fw::Logger::log("[WARNING] %s task name truncated to %s\n", arguments.m_name.toChar(), name);
    }

    CFE_ES_TaskId_t taskId = CFE_ES_TASKID_UNDEFINED;
    CFE_Status_t    cfe_status =
        CFE_ES_CreateChildTask(&taskId, name, CfsTask::childTaskEntry, CFE_ES_TASK_STACK_ALLOCATE,
